...
import {
  FiSearch, FiStar, FiCheckCircle, FiClock,
  FiCalendar, FiBriefcase, FiGlobe, FiLinkedin,
  FiVideo, FiAlertCircle, FiRefreshCw, FiMapPin,
  FiLoader, FiX, FiMessageSquare
} from 'react-icons/fi';
    ...
    <div style={{
      ...
      <div className="glass-card" style={{ width: '100%', maxWidth: '500px', padding: '2rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', color: 'var(--color-muted)', fontSize: '1.3rem', cursor: 'pointer', padding: '0.25rem' }}
          >
            <FiX />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          {recruiter.hourlyFee > 0 && (
            <div style={{
              ...
              <span style={{ fontWeight: 800, fontSize: '1rem', color: '#10B981' }}>${recruiter.hourlyFee}</span>
            </div>
          )}
...
export const FindRecruiters = () => {
  ...
  // Collect all unique tech tags from loaded recruiters
  ...
  const fetchRecruiters = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await recruiterService.getAllRecruiterProfiles();
      setRecruiters(data || []);
    } catch (err) {
  ...
  // Book interview — send to Supabase
      ...
      <div style={{ maxWidth: '1150px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
        ...
        <div className="glass-card" style={{ padding: '1.25rem' }}>
          <div className="filter-bar-grid">
            ...
            {/* Tech filter — dynamic from loaded data */}
            <select
              value={selectedTech}
            ...
            >
              ...
              {allTechTags.map(tag => (
                <option key={tag} value={tag} style={{ background: '#1E1B4B' }}>{tag}</option>
              ))}
            </select>
            ...
            >
              ...
              <option value="newest" style={{ background: '#1E1B4B' }}>🆕 Newest</option>
            </select>
          </div>
        </div>
            ...
            {filteredRecruiters.map((r) => (
              <div key={r.id} className="recruiter-card">
                <div>
                  ...
                  <div className="recruiter-card-header">
                    <div className="recruiter-avatar-box">
                      <img
                        ...
                        alt={r.name}
                        onError={(e) => {
                          e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(r.name)}&background=4f46e5&color=fff&size=128`;
                        }}
                      />
                      <img
                        src={r.companyLogo}
                        ...
                        onError={(e) => {
                          e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(r.company || 'Co')}&background=1abc9c&color=fff&size=32`;
                        }}
                      />
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
                        ...
                        {r.isVerified && <FiCheckCircle style={{ color: '#10B981', fontSize: '0.9rem', flexShrink: 0 }} title="Verified Recruiter" />}
                        {r.isPremiumRecruiter && <FaCrown style={{ color: '#FBBF24', fontSize: '0.9rem', flexShrink: 0 }} title="Premium Recruiter" />}
                      </div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--color-primary)', fontWeight: 600, marginTop: '0.15rem' }}>
                        {r.designation} {r.company && `• ${r.company}`}
                      </div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--color-muted)', marginTop: '0.2rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  ...
                  {/* Bio */}
                  {r.bio && (
                    <p style={{ fontSize: '0.8rem', color: 'var(--color-muted)', margin: '0.75rem 0 0', lineHeight: 1.5,
                  ...
                  {/* Stats Row */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '0.85rem 0', fontSize: '0.8rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: '#FBBF24', fontWeight: 700 }}>
                      <FiStar style={{ fill: '#FBBF24' }} />
                      {r.rating.toFixed(1)}
                      {r.reviewsCount > 0 && <span style={{ color: 'var(--color-muted)', fontWeight: 400 }}>({r.reviewsCount})</span>}
                    </div>
                    <div style={{ color: 'var(--color-muted)' }}>
                      💼 {r.experience}
                    </div>
                    {r.completedInterviews > 0 && (
                      <div style={{ color: 'var(--color-muted)' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', marginBottom: '0.5rem' }}>
                    {(r.techStack || []).slice(0, 5).map((tech, i) => (
                      <span key={i} className="badge-glass" style={{ fontSize: '0.7rem' }}>{tech}</span>
                    ))}
                    {r.techStack?.length > 5 && (
                      <span className="badge-glass" style={{ fontSize: '0.7rem' }}>+{r.techStack.length - 5}</span>
                    )}
                  </div>

                  {/* Links */}
                  {(r.linkedin || r.website) && (
                    <div style={{ display: 'flex', gap: '0.6rem', fontSize: '0.75rem', marginBottom: '0.25rem' }}>
                      {r.linkedin && (
                        <a href={r.linkedin} target="_blank" rel="noopener noreferrer"
                          style={{ color: 'var(--color-primary)', display: 'flex', alignItems: 'center', gap: '0.25rem', textDecoration: 'none' }}>
                ...
                <div style={{ paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '1.15rem', fontWeight: 900, color: '#10B981' }}>
                      ...
                      <span style={{ fontSize: '0.72rem', color: 'var(--color-muted)', fontWeight: 400 }}> / hr</span>
                    </div>
                  </div>
